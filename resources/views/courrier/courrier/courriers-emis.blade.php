...
<div class="col-md-3">
    <div class="form-group">
        <select id="searchByTypeColis"  class="form-control">
...
<table id="table" class="table table-warning table-striped box box-warning"
               ...
               data-search="false" 
               data-toggle="table"
               data-url="{{url('courrier',['action'=>'liste-courriers-emis'])}}"
               data-unique-id="id"
    ...
    <thead>
        <tr>
            ...
            <th data-formatter="destinataireFormatter">Destinataire</th>
            <th data-formatter="contactFormatter">Contact </th>
            <th data-field="traiter" data-formatter="etatFormatter" data-align="center">Envoyer</th>
            ...
            <div class="modal-content">
                <div class="modal-header bg-yellow">
                    ...
                    <span style="font-size: 16px;">
                        ...
                        Gestion des courriers sortant
                    </span>
                </div>
                <div class="modal-body ">
                    <input type="text" class="hidden" name="idCourrier" ng-hide="true" ng-model="courrier.id"/>
                    <input type="text" class="hidden" name="emmettre_recu" ng-hide="true" value="Emis"/>
                    @csrf
                    <div class="row">
                        <div class="col-md-12">
                            <h5 class="text-bold text-green">
                                <label>
                                    <input type="checkbox" id="particulier" name="particulier" ng-model="courrier.particulier" ng-checked="courrier.particulier">&nbsp; Cochez cette case si le courrier est destin&eacute; &agrave; un particulier
                                </label>
                            </h5>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label>Date d'envoi du courrier *</label>
                                <div class="input-group">
                                    <div class="input-group-addon">
                    ...
                    <div class="row">
                     <div class="col-md-12">
                        <h5 class="text-bold text-green">
                            <label>
                                <input type="checkbox" id="traiter" name="traiter" ng-model="courrier.traiter" ng-checked="courrier.traiter">&nbsp; Envoyer ?
                            </label>
                        </h5>
                    </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#searchByDate").change(function (e) {
            ...
            if(date == ''){
                $table.bootstrapTable('refreshOptions', {url: "{{url('courrier', ['action' => 'liste-courriers-emis'])}}"});
            }else{
              $table.bootstrapTable('refreshOptions', {url: '../courrier/liste-courriers-by-date/' + date + '/emis'});
            }
        });
        $("#searchByObjet").keyup(function (e) {
            ...
            if(resultat == ''){
                ...
                $table.bootstrapTable('refreshOptions', {url: '../courrier/liste-courriers-by-objet/' + resultat + '/emis'});
            }
        });
        $("#searchBySociete").change(function (e) {
            ...
            if(societe == 0){
              ...
              $table.bootstrapTable('refreshOptions', {url: '../courrier/liste-courriers-by-societe/' + societe + '/emis'});
            }
        });
        $("#searchByTypeColis").change(function (e) {
            var type = $("#searchByTypeColis").val();
            if(type == 0){
                $table.bootstrapTable('refreshOptions', {url: "{{url('courrier', ['action' => 'liste-courriers-emis'])}}"});
            }else{
              $table.bootstrapTable('refreshOptions', {url: '../courrier/liste-courriers-by-type/' + type + '/emis'});
            }
        });
    ...
    function destinataireFormatter(id, row){
...
//        return row.document_scanner ? '<img width=50 height=50 style="cursor: pointer;" title="Voir le document" onClick="javascript:voirImg(' + row.id + ');" src="{{asset('')}}' + row.document_scanner+'"/>' : "";
    ...
    function editerAnnuaire(methode, url, $formObject, formData, $ajoutLoader) {
        ...
        success:function (reponse, textStatus, xhr){
            if (reponse.code === 1) {
                $.getJSON("../courrier/liste-annuaires/", function (reponse) {
