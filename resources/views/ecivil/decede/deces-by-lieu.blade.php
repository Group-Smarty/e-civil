...
<table id="table" class="table table-warning table-striped box box-warning"
               ...
               data-search="false" 
               data-toggle="table"
               data-url="{{url('e-civil',['action'=>'liste-deces-by-lieu'])}}"
               data-unique-id="id"
               data-show-toggle="false"
               data-show-columns="false">
    <thead>
        <tr>
            <th data-field="lieu_deces" data-formatter="lieuFormatter">Mois </th>
    ...
    $(function () {
        ...
        $("#dateDebut, #dateFin").change(function (e) {
            ...
            if(dateDebut!='' && dateFin!=''){
                $table.bootstrapTable('refreshOptions', {url: '../e-civil/liste-deces-by-lieu-periode/' + dateDebut + '/' + dateFin});
            }else{
                $table.bootstrapTable('refreshOptions', {url: "{{url('e-civil', ['action' => 'liste-deces-by-lieu'])}}"});
            }
        });
    });
    function lieuFormatter(lieu){
        return lieu ? lieu : "Inconnu";
    }
    function imprimePdf(){
        var dateDebut = $("#dateDebut").val();
        var dateFin = $("#dateFin").val();
        if(dateDebut!='' && dateFin!=''){
            window.open("fiche-deces-par-lieu-periode/" + dateDebut + '/' + dateFin ,'_blank');  
        }else{
            window.open("fiche-deces-lieux/" ,'_blank');
        }
    }
