...
<script type="text/javascript">
    function nomFormatter(id, row){
        var tableName = row.Tables_in_ecivil;
        var table = tableName.replace("_",' ');
        return table.toUpperCase();
    }
    
    function optionFormatter(id, row){ 
        if(row.Tables_in_ecivil === 'failed_jobs' || row.Tables_in_ecivil=== 'migrations' || row.Tables_in_ecivil === 'password_resets')
        {
          return '<button class="btn btn-xs btn-danger disabled"><i class="fa fa-stop"></i></button>';  
        }else{
           return '<a class="btn btn-xs btn-success" title="Voir la table" href="../auth/one_table/' + row.Tables_in_ecivil + '"><i class="fa fa-eye"></i></a>';
        }
    }
</script>
@else
